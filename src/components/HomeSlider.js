        ...
        <div className='container'>
            <div className='home-slider'>
                {data.map((item, index) => {
                    return (
                        <div
                            className={`home-slider__item ${index === activeSlide ? 'active' : ''}`}
                            key={index}
                        >
                            <div className='home-slider__item__info'>
                                <h3 className='home-slider__item__info__title'>{item.title}</h3>
                                <p className='home-slider__item__info__description'>
                                    {item.description}
                                </p>
                                <ButtonLink to={`/danh-muc`} className='btn home-slider__btn'>
                                    Đi đến shop
                                    <span>
